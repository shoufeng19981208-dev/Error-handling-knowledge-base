...
- **智能日志匹配** — 整段报错日志直接 Ctrl+V 粘贴即自动匹配：系统反向用每条记录的关键字/标题/内容特征在日志中检索打分（错误码、异常类名、errno、脚本名等），无需用户自己读日志提炼关键词；未命中可一键登记为待处理
- **模糊搜索** — 支持按报错标题、报错内容、关键字、处理步骤、分类进行多字段模糊检索（短关键词自动与智能匹配合并）
- **批量导入** — 首页「批量导入」可下载 xlsx 收集模板（根目录也有 `报错处理知识库-收集模板.xlsx`），线下按行收集报错后一键上传入库；示例行/重复标题自动跳过、逐行报告结果，无处理步骤的记录自动进「待更新」
